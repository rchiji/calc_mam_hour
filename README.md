...
このツールは `gh api` / `gh api graphql` を使って private repo を参照します。
...
### Windows でデスクトップから 1 クリック起動する

このリポジトリには `launch_streamlit.ps1` を入れてあります。

- `.venv\Scripts\python.exe` で `streamlit_app.py` を起動します
- 既に `http://127.0.0.1:8501/` で起動済みなら、サーバーは増やさずブラウザだけ開きます
- 起動に失敗した場合はダイアログを出します

PowerShell から直接起動:

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\launch_streamlit.ps1
```
...
$scriptPath = Join-Path $repoRoot 'launch_streamlit.ps1'
$desktop = [Environment]::GetFolderPath('Desktop')
$linkPath = Join-Path $desktop 'calc_mam_hour Streamlit.lnk'
...
このショートカットをダブルクリックすると Streamlit が起動してブラウザが開きます。
停止したいときは、起動中の PowerShell ウィンドウを閉じるか、Streamlit のプロセスを終了してください。
...
Streamlit 版も CLI と同じ `gh` セッションをそのまま使います。
